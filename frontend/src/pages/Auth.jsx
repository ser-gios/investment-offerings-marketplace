...
import { Eye, EyeOff, TrendingUp, ShieldCheck, Coins } from 'lucide-react';

export default function Auth() {
  const [params] = useSearchParams();
  const [mode, setMode] = useState(params.get('mode') === 'register' ? 'register' : 'login');
  const [form, setForm] = useState({ email: '', password: '', name: '', role: 'investor' });
  ...
  const submit = async (e) => {
    ...
    try {
      console.log('Auth.submit - Mode:', mode, 'Email:', form.email);
      if (mode === 'login') {
        const user = await login(form.email, form.password);
        console.log('Auth.submit - Login successful, redirecting to:', user.role);
        navigate(user.role === 'admin' ? '/admin' : user.role === 'business' ? '/my-projects' : '/marketplace');
      } else {
        console.log('Auth.submit - Registering with:', form.email, form.name, form.role);
        const user = await register(form.email, form.password, form.name, form.role);
        console.log('Auth.submit - Register successful, redirecting');
        navigate(user.role === 'business' ? '/my-projects' : '/marketplace');
      }
    } catch (e) { 
      console.error('Auth.submit - Error:', e.message, e.response?.data);
      setError(e.response?.data?.error || t('auth_error_generic')); 
    }
  ...
  const demoLogin = async (email, pwd) => {
    ...
    try {
      ...
      navigate(user.role === 'admin' ? '/admin' : user.role === 'business' ? '/my-projects' : '/marketplace');
    } catch { setError(t('auth_demo_failed')); }
    finally { setLoading(false); }
  };
        ...
        <div className="card fade-up" style={{ padding: '2.5rem', animationDelay: '0.08s' }}>
          <div style={{ display: 'flex', background: 'var(--obsidian)', borderRadius: 'var(--r-md)', padding: 4, marginBottom: '2rem' }}>
            {['login', 'register'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(''); }} style={{ flex: 1, padding: '0.55rem', borderRadius: 'var(--r-md)', fontSize: '0.875rem', fontWeight: 600, background: mode === m ? 'var(--surface-2)' : 'transparent', color: mode === m ? 'var(--text-primary)' : 'var(--text-muted)', border: mode === m ? '1px solid var(--border)' : '1px solid transparent', transition: 'all 0.18s', cursor: 'pointer' }}>
